...
import {namedBuiltinTypes} from './BuiltinTypes';
...
export default class ServiceParser {
  ...
  /**
   * Parse a definition file, returning an intermediate representation that has
   * all of the information required to generate the remote proxy, as well as
   * marshal and unmarshal the data over a network.
...
class FileParser {
  ...
  /**
   * Helper function that parses an exported function declaration, and returns
   * the function name, along with a FunctionType object that encodes the
  ...
  /**
   * Helper function that parses an exported type alias, and returns the name
   * of the alias, along with the type that it refers to.
   */
